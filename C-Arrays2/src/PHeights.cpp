...
int greatestEleIndex(int *count_arr, int len) {
	int max = 0;
	for (int i = 0; i < len; i++) {
		if (count_arr[i] > max) {
			max = i;
...
void getLeader(int *arr, int len, int *count_arr,int index,int *res) {
	int max = arr[index];
	for (int i = index+1; i < len; i++) {
		if (max < arr[i]) {
			max = arr[i];
			count_arr[i] = count_arr[i] + 1;
			if (count_arr[i] > res[1]) {
				res[1] = count_arr[i];
				res[0] = i;
			}
		}
	...
	if(index!=len-1)
		getLeader(arr, len, count_arr, index + 1,res);
}

int* getLeaderIndex(int *arr, int len){
	if (!arr || len < 0) return NULL;
	int *count_arr = (int *)calloc(len, sizeof(int));
	int *res = (int *)calloc(2, sizeof(int));
	getLeader(arr, len, count_arr,0,res);
	return res;
}